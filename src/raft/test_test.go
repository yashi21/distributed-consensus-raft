...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("1. Disconnected 1 leader %v", leader1)
	cfg.checkOneLeader()
	DPrintf("1. Checked one leader %v", leader1)
	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	DPrintf("1.12 Reconnected one leader %v", leader1)
	leader2 := cfg.checkOneLeader()
	DPrintf("1.13 Checked 2nd leader %v", leader2)
	// if there's no quorum, no leader should
	// be elected.
	cfg.disconnect(leader2)
	cfg.disconnect((leader2 + 1) % servers)
	DPrintf("1.14 Diconnected 2 leaders %v %v", leader2, (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	DPrintf("1.2 Checked no leader")
	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	DPrintf("2. Connected leader %d", (leader2+1)%servers)
	cfg.checkOneLeader()
	DPrintf("2. Checked one leader")

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	DPrintf("3. Disconnected leader %d", leader2)
	cfg.checkOneLeader()
	DPrintf("3. Checked one leader")
	cfg.end()
